...
def ne_rank(text_tf_1,text_tf_2,tfidftransformer_1,vocab_map_1,vocab_map_2):
    ...
    for i in range(iterations):
        ...
        prob = np.array(list(neighbours_score.values()), dtype=float)
        total = np.sum(prob)
        if total != 0:
            prob = prob / total
        current = np.random.choice(list(neighbours_score.keys()), 1, p=prob)[0]
        # toss a coin
    ...
    words = list(ranks.keys())
    ranks = np.array(list(ranks.values()), dtype=float)
    total = np.sum(ranks)
    if total != 0:
        ranks = ranks / total
...
def build_system2(texts_all):
    ...
    vocab_map_2 = {v: k for k, v in tfidftransformer_2.vocabulary_.items()}
    vocab_map_1 = {v: k for k, v in tfidftransformer_1.vocabulary_.items()}
    ...
    for text,text_tf_1,text_tf_2 in zip(texts_all,texts_all_tf_1,texts_all_tf_2):
        ...
        phrases_dict = {}
        for x, phrase in zip(phrases_vectors, phrases):
            x=[final_vec[y] for y in x if y in list(final_vec.keys())]
            avg = np.sum(x)
            phrases_dict[phrase] = avg
        master_phrases_vectors.append(phrases_dict)
...
def use_system1(sow,labmbda,n_results):
    ...
    for text_tf,phrase_dict in zip(texts_all_tf,master_phrases_vectors):
        ...
        keys=list(product_keyword.keys())
        values=list(product_keyword.values())
...
def use_system2(sow,labmbda,n_results):
    ...
    sow_phrase_dict = {}
    for x, phrase in zip(phrases_vectors, phrases):
        x = [sow_final_vec[y] for y in x if y in list(sow_final_vec.keys())]
        avg = np.sum(x)
        sow_phrase_dict[phrase] = avg
    ...
    for nerank_vec,phrase_dict in zip(master_nerank_vectors,master_phrases_vectors):
        ...
        keys = list(product_tf.keys())
        values = list(product_tf.values())
